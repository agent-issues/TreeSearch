...
Where `collapse = TRUE` has contracted a branch, the binary trees it was
contracted from are scored instead, with a warning: the length of a contracted
tree is the length of its best resolution rather than of the edge matrix as
given, which the Sankoff kernel does not compute.
...
It likewise warns, and falls back to the score the search itself compared,
should a returned tree have no finite length at all.
